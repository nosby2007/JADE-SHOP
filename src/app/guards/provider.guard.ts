  ...
  canActivate() {
     return this.afAuth.idTokenResult.pipe(
       switchMap(res => {
         ...
         if (claimRoles.includes('provider')) return of(true);
         ...
         return this.afs.doc(`users/${uid}`).valueChanges().pipe(
           map((doc: any) => Array.isArray(doc?.roles) && doc.roles.includes('[provider]'))
           
         );
       }),
       map(isNurse => {
         if (!isNurse) this.router.navigateByUrl('/login');
         return isNurse;
       })
     );
   }
