...
export class NurseAssessmentsComponent implements OnInit {
  generating = false;
  patientId = '';
   @Input() assessment: any;   // your assessment model
  @Input() prescription: any; // your prescription model
  ...
  try {
    const resp = await this.cloudPrint.printAssessment(this.assessment, /*openAfter*/ false);
    await this.cloudPrint.openPrintResponse(resp); // <— handles url/path/base64 safely
  } catch (e: any) {
...
async printPrescription() {
  try {
    const resp = await this.cloudPrint.printPrescription(this.prescription, /*openAfter*/ false);
    await this.cloudPrint.openPrintResponse(resp);
  } catch (e: any) {
    alert(`Print failed: ${e?.message || e}`);
  }
}
...
async printCustom() {
  try {
    const resp = await this.cloudPrint.printCustom({
      title: 'Admission Packet',
      subtitle: 'Generated from dashboard',
      html: `<p>This is a <strong>custom</strong> block generated by the client.</p>`,
    }, /*openAfter*/ false);
    await this.cloudPrint.openPrintResponse(resp);
  } catch (e: any) {
    alert(`Print failed: ${e?.message || e}`);
