...
export class AssessmentFormComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    orders: this.fb.group({ goalOfCare: ['Healable'] }),
  ...
  constructor(
    ...
    private svc: WoundAssessmentService,
  ...
  // Supprime undefined récursivement
  private stripUndefined<T>(val: T): T {
    if (Array.isArray(val)) {
      return (val.map(v => this.stripUndefined(v)) as unknown) as T;
    }
  ...
  async save() {
    ...
    const payload: WoundAssessment = {
      ...
      // photoURL ajouté après upload si présent
    };

    try {
      // 1) upload éventuel (respecte Storage Option B)
      if (this.file) {
        this.uploading = true;
        const url = await this.svc.uploadPhoto(this.patientId, this.file);
        (payload as any).photoURL = url;
        await this.svc.createMedia(this.patientId, url); // ⚠️ MAJ : envoie les champs requis (dans le service)
      }
    } finally {
    ...
    // 3) redirect
    this.router.navigate(['/skin-wound', this.patientId, 'assessments']);
  }
